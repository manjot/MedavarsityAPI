...
//echo strlen($string)."<br />";
    ...
    function update_save_order()
    {
        $data = array(
            'status'   => $this->_status,
        );
            $this->db->where('order_id', $this->_orderid);
            $this->db->update('subscription_orders', $data);
	}
	...
	function save_order_details()
    {
        ...
        if(!empty($arrsubject)){
            ...
            'hours_remaining'    => $this->_hours,
            );
            $this->db->where('id',$arrsubject['id']);
            $this->db->update('subscription_details', $data);
            return $arrsubject['id'];
        }else{
        $data = array(
            'student_id'     => $this->_user_id,
            'subject_id'     => $this->_subjectid,
	...
	public function check_orderBySubjectId($ids)
	{
        ...
        $this->db->where('sd.student_id', $this->_user_id);
    	...
    	public function studentstate($id)
	{
	
        $this->db->select(array('s.college_name','s.state_id'));
        $this->db->from('medi_registered_students as sd');
        $this->db->join('college_master as s','s.id = sd.college_id');
		$this->db->where('sd.student_id', $id);
        $query = $this->db->get();
        return $query->row_array();
    }

      	public function getstudentstate($id)
	{
	
        $this->db->select(array('sd.id as state_id','sd.state'));
        $this->db->from('state_master as sd');
    ...
    {   
        ...
        $this->db->select(array('*'));
        $this->db->from('subscription_orders');
        $this->db->where('data_added >=',$currentdate);
        $this->db->where('status', 1);
        return $this->db->count_all_results();
    }

     public function getstudentdetailsbyauthtoken()
    {   
        $this->db->select(array('student_id'));
